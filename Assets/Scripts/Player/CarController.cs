...
public class CarController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (IsGrounded())
        {
            rb.AddForce(transform.forward * motorForce * accelerationInput);
        }
    ...
    public void HandleMotor(InputAction.CallbackContext context)
    {
        if (IsGrounded())
        {
            accelerationInput = context.ReadValue<Vector2>().y;
            frontLeftWheelCollider.motorTorque = accelerationInput * motorForce * 5;
            frontRightWheelCollider.motorTorque = accelerationInput * motorForce * 5;
